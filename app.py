...
def remove_from_cart(cart_item_id):
    """Xóa sản phẩm khỏi giỏ hàng (SỬA LẠI CHO CẤU TRÚC 2 BẢNG)"""
    connection = None
    cursor = None
    try:
        connection = mysql.connector.connect(**db_config)
        ...
        cursor.execute('DELETE FROM CartItem_detail WHERE CartItemId = %s', (cart_item_id,))
        ...
        print(f"MySQL Error in remove_from_cart: {db_err}")
        return jsonify({'error': f'Database error: {str(db_err)}'}), 500
    except Exception as e:
        print(f"LỖI CHUNG TRONG remove_from_cart: {e}") 
        return jsonify({'error': f'Error: {str(e)}'}), 500
    finally:
        if cursor:
            cursor.close()
